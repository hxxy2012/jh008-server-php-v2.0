		...
		loginEl.click(function(){
			if(!flag) {   
				...
				validate.run(function(){
					loginEl.text('登录中...');
					server.cityLogin({uName: user, uPass: pwd, rememberMe: rememberMe.length ? 1 : 0}, function(resp) {
						loginEl.text('登录');
						flag = false;
						if (resp.code == 0) {
							var managerRole = resp.body.city_manager.type;
							// 城市id， 没有城市id时候默认为1（成都） 
							var cityId = resp.body.city_manager.city_id || 1;
