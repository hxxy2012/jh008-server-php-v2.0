...
(function(e) {
	...
	p(y.prototype, {
		getSVG: function(a) {
			...
			d = f.exporting.sourceWidth || f.chart.width || /px$/.test(d) && parseInt(d, 10) || 600;
			g = f.exporting.sourceHeight ||
				f.chart.height || /px$/.test(g) && parseInt(g, 10) || 400;
			p(f.chart, {
				animation: !1,
				renderTo: a,
				forExport: !0,
			...
			return d = d.replace(/(url\(#highcharts-[0-9]+)&quot;/g, "$1").replace(/&quot;/g, "'")
		},
		exportChart: function(a, b) {
			var a = a || {}, c = this.options.exporting,
				c = this.getSVG(k({
					chart: {
						borderRadius: 0
					}
				...
				a = k(this.options.exporting, a); alert(a.url);alert(c);
			e.post(a.url, {
		...
		print: function() {
			...
			}), e.appendChild(b), z.focus(), z.print(), setTimeout(function() {
				d.appendChild(b);
				r(g, function(a, b) {
		...
		contextMenu: function(a, b, c, d, e, g, f) {
			...
			c + h.exportMenuWidth > o ?
				a.right = o - c - e - k + "px" : a.left = c - k + "px";
		...
		addButton: function(a) {
			...
			if (a.enabled !== !1) {
				...
				d ? j = function() {
					...
					b.contextMenu("contextmenu", i, f.translateX, f.translateY, f.width, f.height, f);
					f.setState(2)
				});
		...
		destroyExport: function(a) {
			...
			for (b = 0; b < a.exportSVGElements.length; b++)
				if (c = a.exportSVGElements[b]) c.onclick = c.ontouchstart = null, a.exportSVGElements[b] = c.destroy();
