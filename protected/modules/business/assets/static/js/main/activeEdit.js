define(function(require, exports, module) {
	...
	var activeEdit = function() {
		...
		function init(type, datas) {
			...
			setActTags(datas && datas.act_tags || '');
		...
		function getAllTags(fn) { // 获取所有标签
			server.getAllTags(function(resp){
				if (resp.code == 0) {
					fn(resp.body.tags);
				} else {
					tip(resp.msg || '获取标签失败');
		...
		function getValues(callback) { 
			var title, intro, detail, bTime, eTime, bhourTime, ehourTime, addrCity, addrArea, addrRoad, addrNum,
				addrRoute, detailAll, tagIds=[], headImgId, imgIds=[], actId, lon, lat, contactWay,
			...
			//(addrCity = checkItemValues($('#city'), [{reg: /\S{1,}/, message:'城市不能为空'}, {reg: /^\S{1,5}$/, message: '城市不能超过5个字'}])) &&
			(addrCity = checkItemValues($('#city'), '城市不能为空')) &&
			//(addrArea = checkItemValues($('#area'), '区不能为空')) &&
			//(addrRoad = checkItemValues($('#road'),'路不能为空')) &&
			...
			function(){
				detailAll = UM.getEditor('myEditor').getContent();
		...
		el.find('#save').click(function(){ // 保存
			getValues(function(o){
				...
				if (type == 'create') {
					delete parms.actId;
					server.addActivity(parms, function(resp){
						dialog.hide();
						if (resp.code == 0) {
							var d = tip('活动添加成功');
							$('#return')[0].click();
					...
					server.updateActivity(parms, function(resp){
						dialog.hide();
						if (resp.code == 0) {
							tip('保存成功');
							$('#return')[0].click();
