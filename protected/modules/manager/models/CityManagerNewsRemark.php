...
/**
 * This is the model class for table "city_manager_news_remark".
 *
 * The followings are the available columns in table 'city_manager_news_remark':
 * @property string $id
 ...
 */
class CityManagerNewsRemark extends ManagerModel
{
	/**
	...
	{
		return 'city_manager_news_remark';
	}
	...
	public function rules()
	{
		...
		return array(
			...
			array('m_id, news_id', 'length', 'max'=>10),
			array('remark', 'length', 'max'=>256),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, m_id, news_id, remark, status, create_time, modify_time', 'safe', 'on'=>'search'),
		);
	}
	 ...
	 * @return CityManagerNewsRemark the static model class
	 */
     ...
     * @param type $model
     */
    public function addM($mid, $newsId, $remark, $model = NULL)
    {
        if (empty($model)) {
            $model = new CityManagerNewsRemark();
        }
        $model->m_id = $mid;
     ...
     */
    public function remarksM($newsId, $mid, $page, $size) 
    {
        $cr = new CDbCriteria();
